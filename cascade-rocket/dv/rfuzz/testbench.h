...
const int kNumRfuzzCoveragePoints = 2265;

// For Verilator's representation of wide signals, represented as arrays of 32-bit integers.
const int kNumRfuzzCoveragePointsBy32bits = (2265+31)/32;
...
class Testbench {
 public:
  ...
  void initiate_coverage() {
    for (int i = 0; i < kNumRfuzzCoveragePointsBy32bits; i++) {
  ...
  int get_coverage_amount(void) {
    ...
    int num_trailing_bits = kNumRfuzzCoveragePoints % 32;
    for (int bit_id = num_trailing_bits; bit_id < 32; bit_id++) {
      this->coverage_map[kNumRfuzzCoveragePointsBy32bits-1] &= ~(1 << bit_id);
    }
    ...
    assert(ret > 0);
    assert(ret < kNumRfuzzCoveragePoints);
    return ret;
  }
 ...
 private:
  ...
  uint32_t initial_muxsel_vals[(kNumRfuzzCoveragePoints+31)/32]; // We do like Verilator, with wide signals expressed as arrays of 32 bits 
  uint32_t coverage_map[(kNumRfuzzCoveragePoints+31)/32]; // We do like Verilator, with wide signals expressed as arrays of 32 bits 
